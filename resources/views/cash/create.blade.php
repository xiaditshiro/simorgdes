...
@section('content')
    <div class="max-w-3xl mx-auto space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold text-white tracking-wide">Buat Jadwal Kas</h2>
                <p class="text-slate-400 mt-1">Atur jadwal iuran kas baru untuk organisasi.</p>
            </div>
        ...
        <div class="rounded-2xl border border-slate-700/60 bg-[#0b1220]/90 shadow-lg p-6">
            <form action="{{ route('cash.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div>
                    <label class="block mb-2 text-sm text-slate-400">Judul Kas (Contoh: Kas Bulanan Januari)</label>
                    <input type="text" name="title" value="{{ old('title') }}" placeholder="Masukkan judul kas..."
                ...
                <div>
                    ...
                    <input type="number" name="amount" value="{{ old('amount') }}" placeholder="Contoh: 10000"
                        class="w-full rounded-xl border border-slate-700 bg-[#111827] px-4 py-3 text-white placeholder-slate-500 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30">
                </div>
                ...
                <div>
                    ...
                    <button type="button" onclick="addDate()"
                        ...
                        Tambah Tanggal Lainnya
                    </button>
                </div>

                <div>
                    <label class="block mb-2 text-sm text-slate-400">Keterangan Tambahan (Opsional)</label>
                    <textarea name="description" rows="3" placeholder="Tambahkan catatan jika ada..."
                        class="w-full rounded-xl border border-slate-700 bg-[#111827] px-4 py-3 text-white placeholder-slate-500 focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30">{{ old('description') }}</textarea>
                </div>
                ...
                <div class="flex flex-wrap gap-3 pt-4">
                    ...
                    <a href="{{ route('cash.index') }}"
                        class="rounded-xl bg-slate-800 border border-slate-700 px-6 py-3 text-sm font-bold text-slate-300 hover:bg-slate-700 transition text-center">
                        Batalkan
                    </a>
                </div>
            </form>
...
<script>
    ...
    function removeDate(button) {
        if(document.querySelectorAll('#date-wrapper .flex').length > 1) {
            ...
            alert('Minimal harus ada satu tanggal pembayaran.');
        }
    }
</script>
<style>
