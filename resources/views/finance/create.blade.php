...
@section('content')
    ...
    <div class="max-w-2xl mx-auto space-y-6">
        <div class="flex flex-col md:flex-row md:items-center md:justify-between gap-4">
            <div>
                <h2 class="text-3xl font-bold text-white tracking-wide">Tambah Transaksi</h2>
                <p class="text-slate-400 mt-1">Catat pemasukan atau pengeluaran baru organisasi.</p>
            </div>
        ...
        <div class="rounded-2xl border border-slate-700/60 bg-[#0b1220]/90 shadow-lg p-6">
            <form action="{{ route('finance.store') }}" method="POST" class="space-y-5">
                @csrf
                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-5">
                    <div>
                        ...
                        <input type="datetime-local" name="transaction_date" value="{{ old('transaction_date', date('Y-m-d\TH:i')) }}"
                            class="w-full rounded-xl border border-slate-700 bg-[#111827] px-4 py-3 text-white focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30">
                    </div>
                ...
                <div>
                    ...
                    <input type="number" name="amount" value="{{ old('amount') }}" placeholder="Contoh: 50000"
                ...
                <div>
                    ...
                    <textarea name="description" rows="3" placeholder="Tambahkan rincian transaksi..."
                        class="w-full rounded-xl border border-slate-700 bg-[#111827] px-4 py-3 text-white focus:border-cyan-500 focus:outline-none focus:ring-2 focus:ring-cyan-500/30">{{ old('description') }}</textarea>
                </div>
